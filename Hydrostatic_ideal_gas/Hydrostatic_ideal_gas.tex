...
\end{align}
This leads to the conditions on available radii $r$ (for our particular choices of $\mstar$, or $\gasconst$, $r_0$ and $\tmpref_0$) for which the derived atmosphere is valid:
\begin{subequations}
	\begin{align}
	0<r<\frac{\tilde{a}r_0}{\tilde{a}-1}\five &\tilde{a}>1\\
		r>0\five & \tilde{a}\leq1. 
	\end{align}
\label{eq:rvalidad}
\end{subequations}
\section{Polytropes}
...
\end{align}
As for the adiabatic atmosphere (see Equation \eqref{eq:rvalidad}), the choices for $\mstar$, $\gasconst$, $r_0$, $\tmpref_0$, and $n$ determine the range of radii for which the polytrope is valid:
\begin{subequations}
	\begin{align}
		0<r<\frac{ar_0}{a-1}\five & a>1\\
		r>0\five & a\leq1. 
	\end{align}
	\label{eq:rvalidpoly}
\end{subequations}
