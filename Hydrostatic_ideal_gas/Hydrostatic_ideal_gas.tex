...
Note that $b$ is (up to a factor of order unity, depending on convention) equal to the dissipation number ($\di\define \tilde{g}H/(\cp\tilde{T})$, where $H$ is a typical length-scale and the tildes denote typical background-state values). 
