...
\end{align}
Each thermal profile is non-dimensionalized using a representative value from the polytrope. This could be the value of the profile at the outer boundary (e.g., \citealt{Heimpel2022}) or the volume-average of the profile (e.g., \citealt{Hindman2020}). Here instead, we choose the value at the inner boundary (since we have already set $r_0=r\inn$). We thus find

\begin{empheq}[box=\fbox]{align}\label{eq:polynd}
	...
	\andd\hat{\rho}(\hat{r}) &= \left\{  \left[\frac{\beta(1 - e^{-\nrho/n})}{(1-\beta)^2 } \right]  \left(\frac{1}{\hat{r}}\right)  - \left(\frac{ \beta - e^{-\nrho/n} }{1 - \beta} \right)    \right\}^n,\\
	&\ \ \ \ \ \text{(isothermal atmosphere; polytrope with } n\rightarrow\infty). \nonumber
\end{empheq}
%	\frac{1}{\cp}\frac{d\sref}{d\hat{r}} &=   \left(\frac{n-\tilde{n}}{\tilde{n} + 1} \right) \left(\frac{1}{\hat{r}}\right) \frac{1}{1 - \left( \frac{1-\beta}{\beta} \right)   \left( \frac{\beta - e^{-\nrho/n}} {1-e^{-\nrho/n}} \right) \hat{r} }        \\
