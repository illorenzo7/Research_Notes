...
\end{align}
\end{subequations}
$\overleftrightarrow{D}$ is  now defined by Equation \eqref{eq:refmomvisc} and we have redefined $\forcenlbr$ and $\heatnlbr$. %and it is additionally assumed that the fixed reference state satisfies the hydrostatic condition \eqref{eq:egghydro} and the first law of thermodynamics in gradient form, Equation \eqref{eq:anfirstlawref}. 
...
\begin{subequations}\label{eq:eggwtotnke}
	\begin{align}
		\anwtot &\define \anwke + \anwint = \rhoover\left[\frac{1}{2}u^2 + \tmpover(\entrone+\entrover \right]\label{eq:eggwtot}\\
		\andd \anwnke &\define \anwtot - \anwke = \anwint =\rhoover \tmpover (\entrone+\entrover)\label{eq:eggwnke}
	\end{align}
\end{subequations}
