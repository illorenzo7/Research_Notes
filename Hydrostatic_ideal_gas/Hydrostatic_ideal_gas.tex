...
Note that $b$ is closely related to the dissipation number ($\di\define \tilde{g}H/(\cp\tilde{T})$, where $H$ is a typical eq: and the tildes denote typical background-state values). 
...
\begin{align}
	...
	\orr \frac{d\hat{T}}{d\hat{r}} &= \di\hat{g}(\hat{r})\label{eq:dtdr_polytropend},\\
	\where \di &\define \frac{g\topp H}{(n+1)\gasconst \tmpref\topp}\\
...
\begin{align}
	\hat{T}(\hat{r}) &= \frac{\di}{(1-\beta)^2}\frac{1}{\hat{r}} + 1 - \frac{\di}{1-\beta}.
\end{align}
And since $\hat{\rho}(\hat{r}\bcz) = e^{N_\rho}$, we also have 
\begin{align}
	\di &= \beta (e^{N_\rho/n} + 1). 
\end{align}
For the derivatives, note that Equation \eqref{eq:dtdr_polytropend} yields
\begin{align}
	\frac{d\ln\hat{T}}{d\hat{r}} &= -\di \frac{\hat{g}(\hat{r})}{\hat{T}(\hat{r})}\\
	\andd \frac{d^2\ln\hat{T}}{d\hat{r}^2} &= -\left(\frac{d\ln\hat{T}}{d\hat{r}}\right)^2 + \left[\frac{\di}{\hat{T}(\hat{r})}\right] \left[\frac{2\hat{g}(\hat{r})}{\hat{r}} \right],
\end{align}
...
\end{align}
and the $\Delta S$ (which appears in the Rayleigh number) is the already-chosen entropy scale (see \citealt{Heimpel2022}). To specify the non-dimensional entropy gradient properly, we must choose a new non-dimensional parameter,
\begin{align}
	C \define \frac{\cp}{\Delta S} = \frac{1}{\ramod}\frac{g\topp}{\Omega_0^2H}\five\text{or equivalent}.
\end{align}
Then the first law of thermodynamics in Equation \eqref{eq:firstlawtmprho} yields
\begin{align}
	\frac{d\hat{S}}{d\hat{r}} &= C\left[ \frac{1}{\gammaone} \frac{d\ln\hat{T}}{d\hat{r}}   -   \frac{\gammaone - 1}{\gammaone}   \frac{d\ln\hat{\rho}}{d\hat{r}}  \right].
\end{align}
Note that in the current formulation of $\texttt{reference\_type = 2}$, $\d\hat{S}/d\hat{r} = 0$ (hard-coded) and the correct entropy gradient cannot be calculated without defining a new non-dimensional parameter ($C$). 
