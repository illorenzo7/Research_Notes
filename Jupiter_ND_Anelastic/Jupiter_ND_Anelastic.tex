...
\begin{align}
	\frac{d\sref}{dr} &=\psi_{WL}(r; r_0,\delta),
\end{align}
where
\begin{equation}
  \psi_{WL}(r;r_0, \delta)\define \begin{cases}
		0 & r\leq r_0 \\
...
We also define
\begin{equation}
	\psi_{CZ}(r;r_0, \delta)\define 1 - \psi_{WL}(r,r_0-\delta,\delta) \begin{cases}
		1 & r\leq r_0 \\
	...
	\label{eq:dsdr}
\end{equation}
$\psi_{WL}$ thus ``senses" only the WL, and $\psi_{CZ}$ ``senses" only the CZ. 
...
\end{align}
is the polytropic index of the CZ (note that since $\dsdrline\equiv0$ in the CZ, the stratification of the CZ winds up being an adiabatic polytrope). We have also assumed (without loss of generality) that $\sref(r_0)=0$, so that $\sref\equiv0$ in the CZ). 
...
\begin{align}
	\qref\ofr \propto \rhoref\ofr\tmpref\ofr \psi_{CZ}(r; r_0,\delta)
\end{align} 

For the cooling, we choose a cooling width $w_{\rm BL}$:
\begin{align}
	\cref\ofr \propto \exp\left[\frac{r-r_0}{w_{\rm BL}}\right] \psi_{CZ}(r; r_0,\delta)
\end{align}
