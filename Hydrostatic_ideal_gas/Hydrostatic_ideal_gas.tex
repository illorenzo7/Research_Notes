...
\end{align}
where the subscripts mean ``evaluated at $r\inn$ or $r\out$."
...
\end{align}
Each thermal profile is non-dimensionalized using a representative value from the polytrope. This could be the value of the profile at the outer boundary (e.g., \citealt{Heimpel2022}) or the volume-average of the profile (e.g., \citealt{Hindman2020}). Instead, we choose the value at the inner boundary. Plugging everything into Equations \eqref{eq:tmppoly0}--\eqref{eq:spoly0}, we find
...
The non-dimensional polytrope is purely characterized by the three numbers $\beta$, $\nrho$, and $n$. (For completeness, we should also include $\gammaone$, or $\tilde{n}$, but for most cases we are concerned with, $\gammaone=5/3$). Note that the dissipation number $\di$ is \textit{not} an independent parameter. For example, in the non-dimensionalization chosen here, we have

\begin{align*}
	a &= \frac{G\mstar}{(n+1)\gasconst T\inn r\inn} \\
	&= \left(\frac{\beta}{1-\beta}\right) \left(\frac{\gammaone}{\gammaone - 1}\right)  \left( \frac{1}{n+1} \right) \di,\\
	\where \di &\define \frac{g\inn H}{\cp T\inn}
\end{align*}

This, combined with Equation \eqref{eq:afromnond}, yields
\begin{align}
	\di = \left(\frac{\gammaone - 1}{\gammaone}\right)  (n+1)   \left(\frac{1}{\beta} \right) (1 - e^{-\nrho/n}).
\end{align}

Finally, note that Equation \eqref{eq:spolynd} gives the entropy difference across the layer ($\Delta\sref  \define  \sref\out$):
\begin{empheq}[box=\fbox]{align}
	\frac{\Delta\sref}{\cp} &= \left(\frac{\nrho}{\tilde{n}+1}\right)  \left(\frac{n - \tilde{n}}{n} \right). 
\end{empheq}
...
Important: In the current formulation of $\texttt{referece\_type = 3}$ (and in \citealt{Heimpel2022}), $\dialt$ is incorrectly conflated with with $\di$. The reference state is thus physically invalid for all polytropic indices except the adiabatic one, $\tilde{n}=1/(\gammaone - 1)$. 
...
\end{align}
Note that in the current formulation of $\texttt{reference\_type = 3}$, the entropy gradient $d\hat{S}/d\hat{r}$ is hard-coded to zero and the correct entropy gradient cannot be calculated correctly without introducing the new parameter $\brot$ (perhaps call it $\texttt{Buoyancy\_Number}$ in the code). 
...
\bibliographystyle{aasjournal}
  ...
  \begin{figure}
	...
	\caption{Sample of convectively stable polytropes for a solar-like CZ ($n\geq1.5$). The dimensional/geometric values for the polytrope (i.e., $\rm$, $r\topp$, $\rho\bcz$, $M$, $\cp$) are the solar values preceding \eqref{eq:kc_solar}, and $\nrho=3$.}
