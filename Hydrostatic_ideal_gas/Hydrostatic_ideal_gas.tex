...
\end{empheq}
Note that for the isopycnic atmosphere ($n=0$), the entropy difference diverges to $-\infty$.
...
\begin{align}
	...
	&= \frac{\gammaone}{(\gammaone-1)(n+1)}\di \nonumber\\
...
Important: In the current formulation of $\texttt{referece\_type = 3}$ (and in \citealt{Heimpel2022}), $\dialt$ is incorrectly conflated with with $\di$. The reference state is thus physically invalid for all polytropic indices except the adiabatic one, $\tilde{n}=1/(\gammaone - 1)$. I believe the effect is that in the current formulation of the code (which effectively has the incorrect factor $\dialt$ in front of the viscous heating term instead of the correct $\di$), the ``viscous heating" is the true viscous heating multiplied by $(\tilde{n}+1)/(n+1)$. 
...
\begin{subequations}
\begin{align}
	\brot &\define \frac{N\out^2}{\Omega_0^2},\label{eq:buoynum}\\
