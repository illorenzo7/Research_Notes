...
\newcommand{\wint}{(W_1)_{\rm internal}}
\newcommand{\wcomp}{(W_1)_{\rm compression}}
\newcommand{\wpot}{(W_1)_{\rm potential}}
\newcommand{\welast}{(W_1)_{\rm elastic}}
% date, author, title
\date{\today}
...
\begin{document}
	...
	\\
	%(i.e., $\Div(\overline{\rho}\vecu)\equiv0$, where $\overline{\rho}$ is the background density and $\vecu$ the fluid velocity; this takes the place of the $\Div\vecu=0$ condition from the Boussinesq approximation)
	...
	\begin{align}\label{eq:linearthermo}
		\rho = \overline{\rho}(q) + \rho_1(x_i,t)\five &\with \rho_1/\overline{\rho}=O(\epsilon)\ll 1,\\
		P = \prsover(q) + P_1(x_i,t)\five &\with P_1/\prsover=O(\epsilon)\ll1 \nonumber,
	\end{align} 
	and similarly for $T$, $U$, $h$, $\cpcap$, $\mu$, $\delta$, $\cs^2$, $\vecf$, and $Q$. Here, the overbars denote horizontal averages and the ``1" subscripts denote the perturbations about this average. Note that it is \textit{not} correct to write ``$S=\overline{S}(q)+S_1(x_i,t)$ with $S_1/\entrover=O(\epsilon)$." The fully compressible equations of motion contain only differences in entropy and so no meaningful absolute value of $\entrover$ can be defined. Instead, we must write
	\begin{align}\label{eq:linearent}
	S = \entrover(q) + S_1(x_i,t) \five &\with S_1/\cpover=O(\epsilon)\ll 1.
	\end{align} 
	...
	Finally, the vertical convective heat flux, which maximally could transport an energy flux of order $\overline{\rho} \tmpover w \Delta\entrover$, where 
	...
	One consequence of Equation \eqref{eq:nomassflux} is that the horizontally averaged velocity $\overline{\vecu}$ is $O(\epsilon)$ \textit{smaller} than the perturbed velocity $\vecu_1$. For the total mass flux (or equivalently, momentum density), $\bm{m}\define \rho\vecu=(\overline{\rho}+\rho_1)(\overline{\vecu}+\vecu_1)$, we can thus write
		\begin{align}\label{eq:momdensityorig}
		\bm{m} = \bm{m}-\overline{\bm{m}}=\overline{\rho}\vecu_1 + \rho_1\vecu_1 - \overline{\rho_1\vecu_1} + O(\epsilon^2).
	\end{align}
...
\end{align}
	...
	\begin{align}\label{eq:momdensity}
		\bm{m} = \overline{\rho}\vecu + \rho_1\vecu - \overline{\rho_1\vecu} + O(\epsilon^2).
	\end{align}
	Each of the two terms $\rho_1\vecu$ and $-\overline{\rho_1\vecu}$ are $O(\epsilon)$. In most cases, we can thus write $\bm{m} \approx \overline{\rho} \vecu$ to translate from \citet{Gough1969} to the current notation (in which we use $\vecu$ as the primary field variable), except when multiplying by potentially zeroth-order quantities.
	...
	\begin{align}\label{eq:ancont}
		\Div(\overline{\rho}\vecu)\equiv 0,
	\end{align}
	the momentum equation \eqref{eq:mom} becomes 
	\begin{subequations}\label{eq:anmom}
	\begin{align}
		\pderiv{}{t}(\overline{\rho}\vecu)&=-\Div(\overline{\rho}\vecu\vecu) - \nabla P_1 +\rho_1\vecg +\Div \overleftrightarrow{D}+[-\nabla\prsover + \overline{\rho}\vecg],\label{eq:anmommain}\\
	...
	\begin{align}\label{eq:anen}
		\overline{\rho}\cpover\pderiv{T_1}{t} - \deltaover\pderiv{P_1}{t} = &-\overline{\rho}\vecu\cdot\left(\nabla h_1-\frac{1}{\overline{\rho}}\nabla P_1\right) - \overline{\rho} \tmpover \ugrad \entrover\nonumber\\
		& D_{ij}\pderiv{u_i}{x_j} + Q _1- \Div\vecf_1 -\rho_1\vecu\cdot\vecg - \tmpover(\rho_1\vecu-\overline{\rho_1\vecu})\cdot\nabla\entrover\nonumber\\
		&+ [\heatover - \Div\overline{\vecf}],
	\end{align}
	%-\nabla(\overline{\rho} \overline{w^2}) 
  and the equation of state \eqref{eq:eos} becomes linearized, with
  \begin{subequations}\label{eq:aneos}
  \begin{align}
  	\tmpover S_1 &= \cpover T_1 - \frac{\deltaover}{\overline{\rho}} P_1\label{eq:lintp}\\
  	&= U_1 - \frac{\prsover}{\overline{\rho}^2}\rho_1 \label{eq:linu}\\
  	&= h_1 - \frac{P_1}{\rho}\\
  	&= \frac{\cpover\, \tmpover}{\deltaover\overline{\rho}}\left[\frac{P_1}{\cssqover}-\rho_1\right].
  \end{align}
  \end{subequations}
  We have used Equation \eqref{eq:momdensity} to yield the term $- \tmpover(\rho_1\vecu-\overline{\rho_1\vecu})\cdot\nabla\entrover$ in Equation \eqref{eq:anen}. 
  ...
  \begin{align}\label{eq:anhydrostatic}
  	\nabla\prsover\approx \overline{\rho}\vecg
  \end{align}
...
Dotting $\vecu$ into Equation \eqref{eq:anmommain} and using Equation \eqref{eq:anmeanmom} yields the anelastic kinetic energy equation,
	\begin{align}\label{eq:anke}
		\pderiv{}{t}\left(\frac{1}{2}\overline{\rho} u^2\right)&=-\Div\left(\frac{1}{2}\overline{\rho} u^2\vecu \right) - \ugrad P_1 + \rho_1\vecu\cdot\vecg + u_i\pderiv{D_{ij}}{x_j} + \ugrad(\overline{\rho} \overline{w^2}).
	\end{align}
...
\begin{align}\label{eq:anenlhs}
	\overline{\rho}\cpover\pderiv{T_1}{t} - \deltaover\pderiv{P_1}{t} = \overline{\rho}\tmpover\pderiv{S_1}{t}.
\end{align}
...
\begin{align}\label{eq:antote}
			\pderiv{}{t}\left[\overline{\rho}\left(\frac{1}{2} u^2 + \tmpover S_1\right)\right] = &-\Div\left\{\left[\overline{\rho}\left(\frac{1}{2} u^2 + \tmpover S_1\right) + P_1\right]\vecu- \vecu\cdot\overleftrightarrow{D} + \overline{\vecf} + \vecf_1\right\}+ \heatover + Q_1 \nonumber\\
			 &+\left[-\overline{\rho}\tmpover\ugrad\entrover +\ugrad (\overline{\rho} \overline{w^2})- \tmpover(\rho_1\vecu-\overline{\rho_1\vecu})\cdot\nabla\entrover\right].
\end{align}
...
\begin{align}\label{eq:deftote}
	E_{\rm tot} \define \int_V \overline{\rho}\left(\frac{1}{2} u^2 + \tmpover S_1\right) dV,
\end{align}
...
\begin{align}\label{eq:anrhsmom}
	-\nabla P_1 + \rho_1\vecg = -\overline{\rho}\nabla\left(\frac{P_1}{\overline{\rho}}\right) - \deltaover \overline{\rho} \left(\frac{S_1}{\cpover}\right) \vecg + \frac{\deltaover\overline{\rho}}{\cpover} \left(\frac{P_1}{\overline{\rho}}\right)\nabla\entrover.
\end{align}
In the internal energy equation, we find
\begin{align}\label{eq:anrhsen}
	-\overline{\rho}\vecu\cdot\left(\nabla h_1-\frac{1}{\overline{\rho}}\nabla P_1\right) = -\overline{\rho}\tmpover\ugrad S_1 - \overline{\rho} S_1\ugrad\tmpover + \left(\frac{P_1}{\overline{\rho}}\right)\ugrad \overline{\rho}.
\end{align}
...
\begin{align}\label{eq:anrhsen2}
	- \overline{\rho} S_1\ugrad\tmpover + \left(\frac{P_1}{\overline{\rho}}\right)\ugrad S_1 -\rho_1\vecu\cdot\vecg = -\overline{\rho} T_1\ugrad\entrover + O(\epsilon^2).
\end{align}
...
Substituting Equation \eqref{eq:anrhsmom} into Equation \eqref{eq:anmom}, and Equations \eqref{eq:anenlhs}, \eqref{eq:anrhsen}, and \eqref{eq:anrhsen2} into Equation \eqref{eq:anen}, we find
	\begin{align}\label{eq:anmommod}
		\pderiv{}{t}(\overline{\rho}\vecu)= &-\Div(\overline{\rho}\vecu\vecu) -\overline{\rho}\nabla\left(\frac{P_1}{\overline{\rho}}\right) - \deltaover \overline{\rho} \left(\frac{S_1}{\cpover}\right) \vecg + \underbrace{\frac{\deltaover P_1}{\cpover} \nabla\entrover}_{\define \bm{f}_{\rm NLBR}}+\Div \overleftrightarrow{D}\nonumber\\
		&+[-\nabla\prsover + \overline{\rho}\vecg],
	\end{align}
and
\begin{subequations}\label{eq:anenmod}
\begin{align}
	\overline{\rho}\tmpover\pderiv{S_1}{t}= &-\overline{\rho}\tmpover\ugrad S_1 - \overline{\rho} \tmpover \ugrad \entrover \underbrace{- \overline{\rho} T_1\ugrad\entrover}_{\define Q_{\rm NLBR}}+D_{ij}\pderiv{u_i}{x_j} + Q _1- \Div\vecf_1  \nonumber\\
	& +[\heatover - \Div\overline{\vecf} - \tmpover(\rho_1\vecu-\overline{\rho_1\vecu})\cdot\nabla\entrover],\\
	\where \rho_1 =& \frac{P_1}{\cssqover}-\frac{\deltaover\overline{\rho} S_1}{\cpover}\\
	\andd T_1 = & \frac{\tmpover S_1}{\cpover} + \frac{\deltaover P_1}{\overline{\rho}\cpover}.
\end{align}
\end{subequations}
Finally, the mean energy equation \eqref{eq:anmeanen} becomes
\begin{align}\label{eq:anmeanen2}
	\heatover-\Div\overline{\vecf} = \overline{\rho}\tmpover\, \overline{\ugrad S_1}-\overline{D_{ij}\pderiv{u_i}{x_j}} + \overline{\rho} (\overline{T_1\vecu})\dotgrad\entrover 
\end{align}
...
\begin{align}\label{eq:nlbrmom}
	\bm{f}_{\rm NLBR} \define \frac{\deltaover P_1}{\cpover} \nabla\entrover
\end{align}
and the ``non-LBR" heating
\begin{align}\label{eq:nlbrheat}
	Q_{\rm NLBR} \define -\overline{\rho} T_1\ugrad\entrover.
\end{align}
...
\begin{align}
\prsover=	\prsover(q,t) &= \tilde{P}(q) + \hat{P}(q,t) ,\\
\overline{\rho}=	\overline{\rho}(q,t)  &= \tilde{\rho}(q)  + \hat{\rho}(q,t) ,\\
	\entrover = \entrover (q,t) &=  \tilde{S}(q)  + \hat{S}(q,t) ,
\end{align}
etc., where we assume (apart from the entropy) that the hatted means are $O(\epsilon)$ compared to the reference-state means. Note that the horizontal mean profiles are now time-dependent. This does not affect the asymptotic expansion leading to Equations \eqref{eq:ancont}, \eqref{eq:anmommod}, and \eqref{eq:anenmod}, provided the time derivatives of the mean state are the same order as the time derivatives of the deviations [e.g., $\pderivline{\overline{\rho}}{t}=O(\pderivline{\rho_1}{t})$]. Most reference-state profiles are chosen to be time-independent, except for $\tilde{Q}$ and $\tilde{\vecf}$ (see below). 
...
\begin{subequations}\label{eq:aneosref}
	\begin{align}
  	...
  	&= U^\prime - \frac{\prsover}{\overline{\rho}^2}\rho_1 \label{eq:linuref}\\
&= h_1 - \frac{P_1}{\rho}\\
...
\begin{align}\label{eq:nlbrmom2}
	\bm{f}_{\rm NLBR} = \frac{\deltaover P_1}{\cpover} \nabla\tilde{S} + O(\epsilon^2)
\end{align}
...
\overline{\rho}\vecu + \rho_1\vecu - \overline{\rho_1\vecu} = \tilde{\rho}\vecu + \rho^\prime\vecu - \overline{\rho^\prime\vecu} + O(\epsilon^2).
\end{align}
We finally write 
\begin{align}
...
\begin{align}\label{eq:anenrhsfinal}
	-\overline{\rho}\tmpover \ugrad S_1 - \overline{\rho}\tmpover \ugrad\entrover &= - \overline{\rho}\tmpover\ugrad S^\prime -\overline{\rho}\tmpover\ugrad\tilde{S} \nonumber\\
	&=-\tilde{\rho}\tilde{T}\ugrad S^\prime - \tilde{T}(\overline{\rho} \vecu)\cdot\nabla\tilde{S} - \tilde{\rho}\hat{T}\ugrad\tilde{S} + O(\epsilon^2).
\end{align}
...
From Equation \eqref{eq:linu}, the perturbed internal plus potential energy per unit volume is 
\begin{align}\label{eq:perten1}
	W^\prime &= \tilde{U}\rho^\prime + \tilde{\rho} U^\prime + \Phi \rho^\prime \nonumber\\
	&= \underbrace{\tilde{\rho}\tilde{T} S^\prime}_{\define \wint} + \underbrace{\left(\tilde{U}+\frac{\tilde{P}}{\tilde{\rho}}+\Phi\right)\rho^\prime}_{\define\wcomp}.
\end{align}

Reexamining Equation \eqref{eq:aneos}, which is really just an expression of the first law of thermodynamics (which states that changes in the internal energy of fluid parcels come from the sum of external heating and pressure work done by the environment during compression), it is clear that \textit{only} changes in internal energy from heat sources enter into the total energy budget of Equation \eqref{eq:deftote}. The changes in internal energy from density variations ($\overline{U}\rho_1$), the pressure work ($\prsover(\rho_1/\overline{\rho})$), and the changes in potential energy ($\Phi\rho_1$) are all absent. Since all three of these latter effects are due to density variations (i.e., compression or expansion of the fluid), we write
\begin{subequations}\label{eq:perten2}
	\begin{align}
		W_1 &= \wint + \wcomp,\\
		\where \wint&\define \overline{\rho}\tmpover S_1\\
		\andd \wcomp &\define \left(\overline{U}+\frac{\prsover}{\overline{\rho}}+\Phi\right)\rho_1
	\end{align}
\end{subequations}
and note that 
...
%	W_1 = - \frac{\overline{\rho}\Phi}{\cpcap}S_1 + K\frac{P_1}{\cssqover}
%\end{align}
...
%The latter compressive term $(\prsover/\overline{\rho}^2)\rho_1$ is thus eliminated under the anelastic approximation and we instead identify $\tmpover S_1$ with the internal energy. 
