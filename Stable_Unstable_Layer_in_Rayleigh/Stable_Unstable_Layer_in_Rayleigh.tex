...
\begin{subequations}
\begin{align}
	\sigma^2\approx \pr \frac{\ga}{4H\omnought^2}\Sigma\approx \frac{\pr}{f}\Sigma,\\
	\where f\define \frac{R_{\rm eq} - R_{\rm pol}}{R_{\rm eq}}
\end{align}
\end{subequations}
is the geometric oblateness of the object ($R_{\rm eq}$ and $R_{\rm pol}$ refer to the equatorial and polar radius of the object, respectively). We can also estimate
\begin{align}
	\frac{1}{f} \approx \frac{\cpcap}{\Delta S}\rocsq
\end{align}
and thus
\begin{align}
...
In the Sun, we know accurate values of both $\Sigma\sim1$ and $\sigma\sim0.1$ from (dimensional) solar structure models. In Jupiter, where we know very little (even the value of $\sigma$ seems uncertain---we think $\nsqa/4\omnought^2\approx 20$ I think? What is a molecular $\kappa$?), I'm not sure if we could estimate $\Sigma$, which depends on reference-state \textit{gradients}, not just reference-state values. 
 ...
 \end{align}
\end{subequations}
$\heatra\propto \rhotilde\tmptilde$, and $\nutilde=\kappatilde=\etatilde\propto\rhotilde^{-1/2}$. This is the simulation I am working on extending the parameter space with (in particular, to $\sigma<1$). 

For Jupiter, I plan to extend the Wulff simulations (which have, in the CZ-only case,  $\beta=0.8$, $\nrho=1.2$, and$\gamma=3/2$) to:
 \begin{subequations}
	\begin{align}
		\alpha=0.25\\
		\beta=0.8\\
	...
	\end{align}
\end{subequations}
$\heatra\propto \rhotilde\tmptilde$, and $\nutilde=\kappatilde=1$ (constant diffusivities).  If there are any major concerns with these choices, please speak now!
