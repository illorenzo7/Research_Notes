...
\begin{align}
\tmpref\ofr = \tmpref_0 \left[a\bigg{(}\frac{r_0}{r}\bigg{)} + (1 - a)\right]
...
from which
\begin{align}
\prsref(r) = \prsref_0\left[a\bigg{(}\frac{r_0}{r}\bigg{)} + (1 - a)\right]^{n+1},
...
\begin{align}\label{eq:arg2forms}
	a\bigg{(}\frac{r_0}{r}\bigg{)} + (1 - a)&=\frac{b}{n+1}\bigg{(}\frac{r_0}{r}\bigg{)} + \frac{n+1-b}{n+1}.
\end{align}
Putting it all together (and using the ideal gas law to derive the density), we have
	\begin{empheq}[box=\fbox]{align}
\tmpref\ofr &= \tmpref_0\left[\frac{b}{n+1}\bigg{(}\frac{r_0}{r}\bigg{)} + \frac{n+1-b}{n+1}\right] \ \ \ \ \ \text{(polytropic atmosphere)}.\label{eq:tmppolytrope1} \\
\prsref\ofr &= \prsref_0\left[\frac{b}{n+1}\bigg{(}\frac{r_0}{r}\bigg{)} + \frac{n+1-b}{n+1}\right]^{n+1} \ \ \ \ \ \text{(polytropic atmosphere)}. \label{eq:prspolytrope1}\\
\rhoref\ofr &= \rhoref_0\left[\frac{b}{n+1}\bigg{(}\frac{r_0}{r}\bigg{)} + \frac{n+1-b}{n+1}\right]^n \ \ \ \ \ \text{(polytropic atmosphere)}. 
...
\andd \frac{\sref\ofr}{\cp} &= \bigg{(}\frac{n-\tilde{n}}{\tilde{n}+1}\bigg{)}\bigg{\{}\ln{\bigg{(}\frac{r}{r_0}\bigg{)}} - \ln{\left[\frac{b}{n+1}+\frac{n+1-b}{n+1}\bigg{(}\frac{r}{r_0}\bigg{)}\right]}\bigg{\}}\label{eq:spolytrope}\\
...
\prsref\ofr&\rightarrow \prsref_0\exp{\left[\frac{G\mstar}{\gasconst \tmpref_0r_0}\bigg{(}\frac{r_0}{r}-1\bigg{)}\right]},\\
\rhoref\ofr&\rightarrow \rhoref_0\exp{\left[\frac{G\mstar}{\gasconst \tmpref_0r_0}\bigg{(}\frac{r_0}{r}-1\bigg{)}\right]},\\
\frac{1}{\cp}\dsdr &\rightarrow \frac{G\mstar}{\cp\tmpref_0r^2},\\
 \andd \frac{\sref\ofr}{\cp} &\rightarrow \frac{G\mstar}{\cp\tmpref_0r_0}\bigg{(}1-\frac{r_0}{r}\bigg{)}\\ 
...
\begin{align}
	\tmpref\ofr &= T_c\left[a_c\bigg{(}\frac{r_c}{r}\bigg{)} + (1 - a_c)\right],\label{eq:tpolytrope2}\\
	\prsref\ofr &= p_c\left[a_c\bigg{(}\frac{r_c}{r}\bigg{)} + (1 - a_c)\right]^{n+1},\label{eq:ppolytrope2}\\
	\rhoref\ofr &= \rho_c\left[a_c\bigg{(}\frac{r_c}{r}\bigg{)} + (1 - a_c)\right]^n,\label{eq:rhopolytrope2}
\end{align} 
where 
\begin{align}
...
\begin{empheq}[box=\fbox]{align}
\tmpref\ofr &= T_{\rm{m}}\bigg{\{}\left[e^k\tilde{a}\bigg{(}\frac{\rrm}{r}\bigg{)} E_2\bigg{(}\frac{kr}{\rrm}\bigg{)} + (1-e^kE_2(k)\tilde{a})\right]\ekp\bigg{\}},\\
...
Using \eqref{eq:pgeneral} and \eqref{eq:rhogeneral} then yields
	\begin{empheq}[box=\fbox]{align}
\prsref\ofr =\ &p_{\rm{m}}\exp{\left[-\frac{\gamma}{\gamma-1}k\bigg{(}\frac{r}{\rrm} - 1\bigg{)}\right]}\nonumber\\
	&\times\bigg{\{}\left[e^k\tilde{a}\bigg{(}\frac{\rrm}{r}\bigg{)} E_2\bigg{(}\frac{kr}{\rm}\bigg{)} + (1-e^kE_2(k)\tilde{a})\right]\ekp\bigg{\}}^{\gamma/(\gamma-1)}\\
\andd \rhoref\ofr =\ &\rho_{\rm{m}}\exp{\left[-\frac{\gamma}{\gamma-1}k\bigg{(}\frac{r}{\rm}\bigg{)}\right]}\nonumber\\
&\times
\bigg{\{}\left[e^k\tilde{a}\bigg{(}\frac{\rrm}{r}\bigg{)} E_2\bigg{(}\frac{kr}{\rrm}\bigg{)} + (1-e^kE_2(k)\tilde{a})\right]\ekp\bigg{\}}^{1/(\gamma-1)}\\
\five &\text{(constant entropy gradient).}\nonumber
\end{empheq}
...
\begin{align}
\sref\ofr &= \frac{k\cp}{\rrm}\bigg{\{}\frac{1}{2}{\left[(r-\rrm) -\delta\ln\cosh{\bigg{(}\frac{r-\rrm}{\delta}\bigg{)}}\right]\bigg{\}}}\label{eq:s_tanh_transition}\\
\frac{ds}{dr} &= \frac{k\cp}{\rrm}\bigg{\{}\frac{1}{2}\left[1 - \tanh{\bigg{(}\frac{r-\rrm}{\delta}\bigg{)}}\right]\bigg{\}}.\label{eq:dsdr_tanh_transition}\\
