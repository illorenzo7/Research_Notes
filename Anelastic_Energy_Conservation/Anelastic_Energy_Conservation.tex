...
%\end{align}
Substituting Equation \eqref{eq:anmomrhsref} into Equation \eqref{eq:anmommod} and noting that all terms are of $O(\epsilon)$ (so that we can replace overbars with tildes), we find
...
\begin{align}
	...
	\text{where we note}\five \rhoprime &= \frac{\prsprime}{\cssqtilde}-\frac{\deltatilde\rhotilde \entrprime}{\cptilde}\label{eq:eggenrhoprime}\\
\andd \tmpprime &=  \frac{\tmptilde \entrprime}{\cptilde} + \frac{\deltatilde \prsprime}{\rhotilde\cptilde}.\label{eq:eggentmpprime}
%\end{empheq}
\end{align}
\end{subequations}
 $\overleftrightarrow{D}$ is  now defined by Equation \eqref{eq:refmomvisc}. %and it is additionally assumed that the fixed reference state satisfies the hydrostatic condition \eqref{eq:egghydro} and the first law of thermodynamics in gradient form, Equation \eqref{eq:anfirstlawref}. 
