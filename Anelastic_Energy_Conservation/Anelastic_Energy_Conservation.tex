...
\begin{empheq}[box=\fbox]{align}\label{eq:reftote}
...
\end{empheq}
Again using condition \eqref{eq:nomeanu} (or equivalently, Equation \eqref{eq:refcont} and assuming $w\equiv0$ on the boundaries), integrating Equation \eqref{eq:reftote} over $V$, using the divergence theorem, and assuming that the sum of the surface-integrated fluxes balances the volume-integrated heating yields
...
\where \eggwtot &\define \eggwke + \eggwheat,\\
 ...
 \andd \eggwheat &\define \rhotilde\tilde{T} \entrprime.
\end{empheq}
\end{subequations}
This conservation law holds for arbitrary fluid motions obeying Equations \eqref{eq:anegg} and for all magnitudes of $|\nabla\entrover|$. 
...
\end{align}
in the internal energy equation. Alternatively, if the LBR approximation is made, (setting $\tilde{\bm{f}}_{\rm NLBR}\equiv0$ in Equation \eqref{eq:refmom}), energy can still be conserved if $\tilde{Q}_{\rm NLBR}$ in Equation \eqref{eq:refen} is replaced by
