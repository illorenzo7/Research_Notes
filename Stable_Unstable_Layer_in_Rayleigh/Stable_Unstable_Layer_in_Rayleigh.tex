...
We leave the time-scale general for now, but ultimately assume it is a rotational time-scale when running the simulations [i.e., $\tau=(2\Omega_0)^{-1}$]. Leaving $\tau$ general allows easy translation between different nondimensionalizations, if they are needed later. %To describe the reference state, we will consider three cases for a given function's ``typical value": Its value at the inner shell boundary, its value at the outer shell boundary, or its value volume-averaged over the shell. 
	...
	\begin{align}
		...
		\sigma &\define \sqrt{\frac{\taues}{\taunu}} =\frac{N_{\rm a}}{2\omnought}\sqrt{\pr}\five\text{(``$\sigma$ parameter")}
		\end{align}
	\end{subequations}
...
\begin{align}\label{eq:enndvisc}
	...
	& + \frac{1}{\pr}\tauovertaunu\heattilde + \frac{\pr\di}{\ra}\left(\frac{\taunu}{\tau}\right)\left( D_{ij}e_{ij} + \frac{1}{\prm} \etatilde|\curl\vecb|^2\right),
\end{align}
...
\begin{subequations}
	\begin{align}
		...
		\andd \ra &\define \frac{\ga \tilde{Q} H^3\tau}{\tilde{\rho}\tilde{T}\cp\nua \kappaa} =  \frac{\ga \tilde{Q} H^3}{2\Omega_0\tilde{\rho}\tilde{T}\cp\nua \kappaa}.\label{eq:rarot}
	\end{align}
\end{subequations}
...
\begin{subequations}
	\begin{align}
		\ramod &\define \frac{\ek^2}{\pr}\ra =  \frac{\ga }{H\Omega_0^2} \frac{\Delta S}{\cp}=\frac{\ga\tilde{Q}}{(2\Omega_0)^3\tilde{\rho}\tilde{T}\cp H},\\ 
		\andd \bumod &\define \frac{\ek^2}{\pr}\bu = \frac{\nsqa} {4\Omega_0^2} \sim \frac{\ga }{H\Omega_0^2} =  \frac{1}{\text{geometric\ oblateness}}.
	\end{align}
\end{subequations}
...
\begin{align}\label{eq:digory}
	\di \define \frac{\ga H}{\cp\tilde{T}} &=  \frac{3 \beta (1 - \beta)^2 (1 - e^{-\nrho/n})} 
