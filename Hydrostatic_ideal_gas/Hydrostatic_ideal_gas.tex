...
\label{eq:rhogeneral}
	...
	\begin{empheq}[box=\fbox]{align}
	\prsref\ofr &= \prsref_0 \left[\tilde{a}\parenfrac{r_0}{r} + (1-\tilde{a})\right]^{\gammaone/(\gammaone-1)} \ \ \ \ \ \text{(adiabatic atmosphere)}
\label{eq:prsad}\\
\andd	\rhoref\ofr &= \rhoref_0 \left[\tilde{a}\parenfrac{r_0}{r} + (1-\tilde{a})\right]^{1/(\gammaone-1)} \ \ \ \ \ \text{(adiabatic atmosphere).}
\label{eq:rhoad}
\end{empheq}
...
\label{eq:tmppolytrope0}
\end{align}
where the new constant $a$ is dimensionless. Note the similarity of the argument $[a(r_0/r) + (1-a)]$ in Equation \eqref{eq:tmppolytrope0} (which was derived by assuming a divergenceless temperature gradient) to the argument in Equation \eqref{eq:tmpad} (which was derived by assuming the gas was adiabatic). 
...
\rhoref\ofr &= \rhoref_0\left[\frac{b}{n+1}\left(\frac{r_0}{r}\right) - \frac{b-(n+1)}{n+1}\right]^n \ \ \ \ \ \text{(polytropic atmosphere)}. \label{eq:rhopolytrope1}
\end{empheq}
...
Note that for the polytrope, in addition to the two integration constants $\prsref_0$ and $\tmpref_0$, there is another constant $n$ (or equivalently, $a$). Specifying $n$ is the same as specifying $\dsdrline$. For, plugging Equations \eqref{eq:tmppolytrope1}--\eqref{eq:rhopolytrope1} (we only need two of them) into any form of \eqref{eq:firstlaw3} yields
\begin{subequations}
\begin{empheq}[box=\fbox]{align}
...
\label{def:n0}
\end{align}
From \eqref{eq:dsdrpolytrope}, it is obvious that $n=\tilde{n}$ (or $a=\tilde{a}$) corresponds to an adiabatic atmosphere (as we have already shown in \eqref{eq:tmpad}--\eqref{eq:rhoad}), while
\begin{align}
...
Recalling that $\lim_{n\rightarrow\infty}(1+x/n)^n=\exp(n)$, Equations \eqref{eq:tmppolytrope1}--\eqref{eq:dsdrpolytrope} show that the limit $n\rightarrow\infty$ simply corresponds to an isothermal atmosphere:
...
For an adiabatic, polytropic convection zone (CZ) sitting atop a stably stratified radiative zone (RZ), it make sense to choose the reference radius to be the boundary between the RZ and CZ, i.e., $r_0=r\bcz$. In the work of \citet{Jones11} and in the Rayleigh code \citep{Featherstone18}, the polytrope is initiated using the density at the base of the CZ ($\rhoref\bcz$) and the number of density scale heights $N_\rho$ across the CZ , as opposed to the two base-of-CZ values $\prsref\bcz$ and $\tmpref\bcz$. Here $N_\rho\coloneqq\ln{(\rhoref\bcz/\rhoref\topp)}$, where $\rhoref\topp$ is the density at the top of the shell (outer boundary). We can recast Equations \eqref{eq:tmppolytrope1}--\eqref{eq:rhopolytrope1} in terms of $\rhoref\bcz$ and $N_\rho$ by using \eqref{eq:polytropicindex} and \eqref{eq:rhopolytrope1} to yield
\begin{align}
...
\citet{Jones11} define the polytrope using the variables at the center of the CZ (i.e., in our notation $r_0=r_c\coloneqq(r\bcz+r_{\rm{o}})/2$). In that case, \eqref{eq:tmppolytrope1}--\eqref{eq:rhopolytrope1} become
\begin{align}
	\tmpref\ofr &= T_c\left[a_c\left(\frac{r_c}{r}\right) + (1 - a_c)\right],\label{eq:tmppolytrope2}\\
...
\andd \tilde{\zeta}(r) &\coloneqq a_c\left(\frac{r_c}{r}\right) + (1-a_c) = c_0 + c_1\left(\frac{d}{r}\right),\label{def:tildezeta}
\end{align}
\eqref{eq:tmppolytrope2}--\eqref{eq:rhopolytrope2} become
\begin{align}
...
\begin{align}
\tilde{\zeta}_o \coloneqq \tilde{\zeta}(r_{\rm{o}}) = \frac{1+\beta}{\beta e^{N_\rho/n} + 1}, 
\end{align}
then 
\begin{align}
c_0 = \frac{2\tilde{\zeta}_o - \beta - 1}{1-\beta} \ \ \ \ \ \andd c_1 = \frac{(1+\beta)(1-\tilde{\zeta}_o)}{(1-\beta)^2},
\label{eq:c01_from_zeta0}
\end{align}
yielding exactly the formulation for the polytrope in Jones et al. (2011). 

Both the formulation with respect to the center of the shell \eqref{def:tildezeta}--\eqref{eq:c01_from_zeta0} and the formulation with respect to the middle boundary using \eqref{eq:tmppolytrope1}--\eqref{eq:rhopolytrope1} (also \eqref{eq:ti_from_nrho} if specifying $\rhoref\bcz$ and $N_\rho$ instead of $p_{\rm{m}}$ and $\tmpref\bcz$) are mathematically equivalent. However, given the substantial extra notation involved in defining the constants $c_0$ and $c_1$, it seems preferable to use the latter formulation. 
