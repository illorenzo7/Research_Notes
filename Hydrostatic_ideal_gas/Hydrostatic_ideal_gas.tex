...
\begin{align}
\tmpref\ofr = \tmpref_0 \bigg{[}a\bigg{(}\frac{r_0}{r}\bigg{)} + (1 - a)\bigg{]}
...
from which
\begin{align}
\prsref(r) = \prsref_0\bigg{[}a\bigg{(}\frac{r_0}{r}\bigg{)} + (1 - a)\bigg{]}^{n+1},
...
Putting it all together (and using the ideal gas law to derive the density), we have
	\begin{empheq}[box=\fbox]{align}
\tmpref\ofr &= \tmpref_0\bigg{[}\frac{b}{n+1}\bigg{(}\frac{r_0}{r}\bigg{)} + \frac{n+1-b}{n+1}\bigg{]} \ \ \ \ \ \text{(polytropic atmosphere)}. \\
\prsref\ofr &= \prsref_0\bigg{[}\frac{b}{n+1}\bigg{(}\frac{r_0}{r}\bigg{)} + \frac{n+1-b}{n+1}\bigg{]}^{n+1} \ \ \ \ \ \text{(polytropic atmosphere)}. \\
\rhoref\ofr &= \rhoref_0\bigg{[}\frac{b}{n+1}\bigg{(}\frac{r_0}{r}\bigg{)} + \frac{n+1-b}{n+1}\bigg{]}^n \ \ \ \ \ \text{(polytropic atmosphere)}. 
\label{eq:rhopolytrope1}
\end{empheq}

As for the adiabatic atmosphere (see Equation \eqref{eq:rvalidad}), the choices for $a$ (i.e., $b$---via $\mstar$, $\gasconst$, and $\tmpref_0$---and $n$) and $r_0$ determine the range of radii for which the polytrope is valid:
\begin{subequations}
	\begin{alignat}{2}
		0<\ &r<\left[\frac{b}{b-(n+1)}\right]r_0\five && b=\frac{G\mstar}{\gasconst \tmpref_0 r_0} >n+1\\
...
\andd \frac{\sref\ofr}{\cp} &= \bigg{(}\frac{n-\tilde{n}}{\tilde{n}+1}\bigg{)}\bigg{\{}\ln{\bigg{(}\frac{r}{r_0}\bigg{)}} - \ln{\bigg{[}\frac{b}{n+1}+\frac{n+1-b}{n+1}\bigg{(}\frac{r}{r_0}\bigg{)}\bigg{]}}\bigg{\}}\label{eq:spolytrope}\\
...
\end{align}
Furthermore, we can compute, at any particular radius $r$:
\begin{align*}
	\pderiv{}{n}\left(\dsdr\right)=  \frac{b}{\tilde{n}+1}\left(\frac{r_0}{r^2}\right)  \left\{\frac{\tilde{n}+1-b+b(r_0/r)}{\left[n+1-b+b(r_0/r)\right]^2}\right\}
\end{align*}
