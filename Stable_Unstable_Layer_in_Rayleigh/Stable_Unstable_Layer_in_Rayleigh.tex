...
\end{subequations}
	...
	\end{align}
\end{subequations}
The only difference between Jupiter and the tachocline is which of the CZ or RZ lies above the other one. 

We leave the time-scale general for now, but ultimately assume it is a rotational time-scale [i.e., $\tau=(2\Omega_0)^{-1}$]. %To describe the reference state, we will consider three cases for a given function's ``typical value": Its value at the inner shell boundary, its value at the outer shell boundary, or its value volume-averaged over the shell. 
